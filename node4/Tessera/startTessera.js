...
const startTessera = () => {
    ...
    const dockerCommand = `docker run -v ${currentDir}:/app -p 9104:9104 --name tessera-node4 --rm quorumengineering/tessera:latest -configfile /app/config.json`;
    ...
    process.on('SIGINT', () => {
        ...
        exec('docker stop tessera-node4', (error) => {
            if (error) {

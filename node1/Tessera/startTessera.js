...
const startTessera = () => {
    ...
    const dockerCommand = `docker run -v ${currentDir}:/app -p 9101:9101 --name tessera-node1 --rm quorumengineering/tessera:latest -configfile /app/config.json`;
    ...
    process.on('SIGINT', () => {
        ...
        exec('docker stop tessera-node1', (error) => {
            if (error) {

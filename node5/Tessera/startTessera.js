...
const startTessera = () => {
    ...
    const dockerCommand = `docker run -v ${currentDir}:/app -p 9105:9105 --name tessera-node5 --rm quorumengineering/tessera:latest -configfile /app/config.json`;
    ...
    process.on('SIGINT', () => {
        ...
        exec('docker stop tessera-node5', (error) => {
            if (error) {

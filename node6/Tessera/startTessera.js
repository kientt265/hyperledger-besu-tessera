...
const startTessera = () => {
    ...
    const dockerCommand = `docker run -v ${currentDir}:/app -p 9106:9106 --name tessera-node6 --rm quorumengineering/tessera:latest -configfile /app/config.json`;
    ...
    process.on('SIGINT', () => {
        ...
        exec('docker stop tessera-node6', (error) => {
            if (error) {

...
const startTessera = () => {
    ...
    const dockerCommand = `docker run -v ${currentDir}:/app -p 9103:9103 --name tessera-node3 --rm quorumengineering/tessera:latest -configfile /app/config.json`;
    ...
    process.on('SIGINT', () => {
        ...
        exec('docker stop tessera-node3', (error) => {
            if (error) {
